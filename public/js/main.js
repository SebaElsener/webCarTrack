...
window.addEventListener("load", async () => {
  ...
  if (userCart == "") {
    cartLinkSpan[0].innerHTML = ": VACIO";
  } else {
...
for (let i = 0; i < deleteBtn.length; i++) {
  deleteBtn[i].addEventListener("click", () => {
    productId =
      updateBtn[i].parentElement.previousElementSibling.childNodes[1].id;
...
for (let i = 0; i < updateBtn.length; i++) {
  updateBtn[i].addEventListener("click", () => {
    ...
    productToUpdate = {
      ...
      price:
        updateBtn[i].parentElement.parentElement.childNodes[3].childNodes[3]
          .value,
      stock:
        updateBtn[i].parentElement.parentElement.childNodes[9].childNodes[3]
          .value,
      ...
      thumbnail:
        updateBtn[i].parentElement.parentElement.childNodes[5].childNodes[1]
          .src,
    };
    fetch(`/api/productos/${productId}`, {
      method: "PUT",
...
for (let i = 0; i < buyBtn.length; i++) {
  buyBtn[i].addEventListener("click", async () => {
    ...
    await fetch("/api/productos/arrayproductos")
      .then((res) => res.json())
      .then(async (productos) => {
        const selectedProduct = productos.find(
          (product) => product.id === buyBtn[i].id,
        );
        ...
        if (!cartLink[0].id) {
          await fetch("/api/carrito", { method: "POST" })
            ...
            .then(async (newCart) => {
              ...
              cartId = newCart._id;
              const userIdAndCartId = {
                userId: userWelcome[0].id,
                cartId: cartId,
              };
